...
public class Scale2 {
    ...
    public static void scaleAsPerBinPack(int currentsize) {
        log.info("Currently we have this number of consumers for cg 2 {}", currentsize);
        int neededsize = binPackAndScale();
        size= neededsize;
        log.info("We currently need the following consumers (as per the bin pack) for cg 2 {}", neededsize);
        ...
        if (replicasForscale == 0) {
          ...
          /*  if(!doesTheCurrentAssigmentViolateTheSLA()) {
            ...
            log.info("We have to upscale cg2 by {}", replicasForscale);
            ...
            try (final KubernetesClient k8s = new DefaultKubernetesClient()) {
                k8s.apps().deployments().inNamespace("default").withName("cons1persec2").scale(neededsize);
                log.info("I have Downscaled cg2 you should have {}", neededsize);
